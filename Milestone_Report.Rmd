...
With a more manageable sampled subset of the data, we briefly look at some of the more / challenging (for our project) features of the data. In order to get an idea of such features, we  look at the last 5 lines in the sample drawn from the blogs sample.
...
For the purposes of text prediction, these 10 lines reveal the presence of several types of characters that should be eliminated as they shouldn't be predicted. These include punctuation, quotation marks and digits. Furthermore, this and the three other samples are likely to contain non-English characters, emoticons, curse words and other things we wouldn't like to predict. As such, we clean the samples. The current report illustrates this cleaning, and subsequent analyses, with the **blogs** corpus sample. The following block of code does so, displaying the first 5 lines of the _cleaned_ blog sample. 
...
par(mfrow=c(2,2))
plot(blogs_freq_tab_1_sub.plot); plot(blogs_freq_tab_2_sub.plot); plot(blogs_freq_tab_3_sub.plot); plot(blogs_freq_tab_4_sub.plot)
