...
TOKENS WITH C(W) <= 5 BECOME "UNK", CALCULATE PS FOR UNIGRAMS
...
## Blogs
# Read in, replace C(W) <= 5 w/ "UNK", calculate ps
...
blogs.corpus_tokC1 <- blogs.corpus_tokC[token.count <= 5, token := "UNK"]
blogs.corpus_tokC2 <- blogs.corpus_tokC1[,p := round((token.count / nrow(blogs.corpus_tokC1)), digits = 7)]
blogs.corpus_tokC2[, 2 := NULL]

blogs.corpus_tokC2[, token.index:=.GRP, by = token]

# Write p-values, indices and words -> indices to file
blogs.tok_ps <- blogs.corpus_tokC2[, p]
write.table(blogs.tok_ps,"blogs.pvals.csv", sep=",", row.names = FALSE)
blogs.tok_ind <- blogs.corpus_tokC2[, token.index]
write.table(blogs.tok_ind,"blogs.indices.csv", sep=",", row.names = FALSE)
blogs.tok_wordind <- blogs.corpus_tokC2[,. (token,token.index)]
write.table(blogs.tok_wordind,"blogs.wordindices.csv", sep=",", row.names = FALSE)

# Clean up
rm(blogs.corpus_tok); rm(blogs.corpus_tokC); rm(blogs.corpus_tokC1); rm(blogs.corpus_tokC2); rm(blogs.tok_ps); rm(blogs.tok_ind); rm(blogs.tok_wordind)
...
news.corpus_tokC1 <- news.corpus_tokC[token.count <= 5, token := "UNK"]
news.corpus_tokC2 <- news.corpus_tokC1[,p := round((token.count / nrow(news.corpus_tokC1)), digits = 7)]
news.corpus_tokC2[, 2 := NULL]

news.corpus_tokC2[, token.index:=.GRP, by = token]

# Write p-values, indices and words -> indices to file
news.tok_ps <- news.corpus_tokC2[, p]
write.table(news.tok_ps,"news.pvals.csv", sep=",", row.names = FALSE)
news.tok_ind <- news.corpus_tokC2[, token.index]
write.table(news.tok_ind,"news.indices.csv", sep=",", row.names = FALSE)
news.tok_wordind <- news.corpus_tokC2[,. (token,token.index)]
write.table(news.tok_wordind,"news.wordindices.csv", sep=",", row.names = FALSE)

# Clean up
rm(news.corpus_tok); rm(news.corpus_tokC); rm(news.corpus_tokC1); rm(news.corpus_tokC2); rm(news.tok_ps); rm(news.tok_ind); rm(news.tok_wordind)
...
tweets.corpus_tokC1 <- tweets.corpus_tokC[token.count <= 5, token := "UNK"]
tweets.corpus_tokC2 <- tweets.corpus_tokC1[,p := round((token.count / nrow(tweets.corpus_tokC1)), digits = 7)]
tweets.corpus_tokC2[, 2 := NULL]

tweets.corpus_tokC2[, token.index:=.GRP, by = token]

# Write p-values, indices and words -> indices to file
tweets.tok_ps <- tweets.corpus_tokC2[, p]
write.table(tweets.tok_ps,"tweets.pvals.csv", sep=",", row.names = FALSE)
tweets.tok_ind <- tweets.corpus_tokC2[, token.index]
write.table(tweets.tok_ind,"tweets.indices.csv", sep=",", row.names = FALSE)
tweets.tok_wordind <- tweets.corpus_tokC2[,. (token,token.index)]
write.table(tweets.tok_wordind,"tweets.wordindices.csv", sep=",", row.names = FALSE)

# Clean up
rm(tweets.corpus_tok); rm(tweets.corpus_tokC); rm(tweets.corpus_tokC1); rm(tweets.corpus_tokC2); rm(tweets.tok_ps); rm(tweets.tok_ind); rm(tweets.tok_wordind)

```

MAKE BIGRAMS OUT OF INDICES

```{r make bigrams}

# Load in data in indices, switch to matrix, transpose and group by 2
blogs.indices <- fread("blogs.indices.csv")
blogs.indices1 <- t(blogs.indices)
blogs.bi <- split(blogs.indices1, ceiling(seq_along(blogs.indices1)/2))

news.indices <- fread("news.indices.csv")
news.indices1 <- t(news.indices)
news.bi <- split(news.indices1, ceiling(seq_along(news.indices1)/2))

tweets.indices <- fread("tweets.indices.csv")
tweets.indices1 <- t(tweets.indices)
tweets.bi <- split(tweets.indices1, ceiling(seq_along(tweets.indices1)/2))

# Clean up
rm(blogs.indices); rm(blogs.indices1); rm(news.indices); rm(news.indices1); rm(tweets.indices); rm(tweets.indices1)

# Write bigrams to files
write.table(blogs.bi,"blogs.bi.csv", sep=",", row.names = FALSE)
write.table(news.bi,"news.bi.csv", sep=",", row.names = FALSE)
write.table(tweets.bi,"tweets.bi.csv", sep=",", row.names = FALSE)
...
## Blogs
# Load in bigranms, eliminate bigrams with n <= 3
