...
## Get individual word counts in bigrams
blogs.wi_1count <- count(blogs.bi_tab2, c("wi_1")); blogs.wicount<- count(blogs.bi_tab2, c("wi")); blogs.wcount <- blogs.wi_1count$freq + blogs.wicount$freq

news.wi_1count <- count(news.bi_tab2, c("wi_1")); news.wicount<- count(news.bi_tab2, c("wi")); news.wcount <- news.wi_1count$freq + news.wicount$freq

tweets.wi_1count <- count(tweets.bi_tab2, c("wi_1")); tweets.wicount<- count(tweets.bi_tab2, c("wi")); tweets.wcount <- tweets.wi_1count$freq + tweets.wicount$freq


## Add in columns with bigram probabilities

blogs.bi_tab3 = data.frame(blogs.bi_tab2, blogs.wcount); names(blogs.tri_tab2) <- c(x_name, y_name, z_name, a_name)





blogs.bi_tab3 <- transform(blogs.bi_tab3, w.count = wi_1.count + wi.count); blogs.bi_tab3 <- transform(blogs.bi_tab3, p = bigram.count / w.count)

news.bi_tab3 <- as.data.frame(news.bi_tab2.2); news.bi_tab3 <- transform(news.bi_tab3, w.count = wi_1.count + wi.count); news.bi_tab3 <- transform(news.bi_tab3, p = bigram.count / w.count)


blogs.bi_tab2.1 <- transform(blogs.bi_tab2.1, p = bigram.count / wi_1.count)
news.bi_tab2.1 <- as.data.frame(news.bi_tab2.1); news.bi_tab2.1 <- transform(news.bi_tab2.1, bigram.p = bigram.count / target.count)
tweets.bi_tab2.1 <- as.data.frame(tweets.bi_tab2.1); tweets.bi_tab2.1 <- transform(tweets.bi_tab2.1, bigram.p = bigram.count / target.count)

## Finally, take out target.count and bigram.count columns 
blogs.bi_tab2.2 <- subset(blogs.bi_tab2.1, select = -c(target.count, bigram.count) )
news.bi_tab2.2 <- subset(news.bi_tab2.1, select = -c(target.count, bigram.count) )
tweets.bi_tab2.2 <- subset(tweets.bi_tab2.1, select = -c(target.count, bigram.count) )


### For trigrams, split "grams" into wi, w(i-1) and w(i-2), then add probablity columns to frequency tables
x_name <- "wi-2"; y_name <- "wi-1"; z_name <- "wi"; a_name <- "trigram.count"
...
### Use data.table to add in counts of w(i) - 1 and w(i)-2
